...
Two ways to get the ai-o11y-demo-apps dashboards in your Grafana:
...
## Option B — Import the JSON directly (faster, needs API token)
...
This POSTs every `.json` file in this directory to your Grafana via
the dashboards HTTP API.

(Currently the prompt path is the primary one — JSON snapshots are
generated by the Assistant after first creation.)
