...
// to their target traffic slices, matched per-agent via the "Agent name" criteria.
//
...
//   { rule_id, enabled, selector, match: {"tags.gen_ai.agent.name": "<agent>"}, sample_rate: 0-1, evaluator_ids[] }
// The Sigil UI renders this match key as "Agent name" criteria.
//
...
(async () => {
  ...
  // ── Cleanup: nuke any prior per-app rules from the interim attempt. Safe to
  // re-run — 404s on missing IDs are silently counted as "already gone".
  const STALE_IDS = [
  ...
  console.log(`%cDeleting ${STALE_IDS.length} interim per-app rules…`,
  ...
  // span multiple agents, we emit one rule per agent so Sigil's per-agent
  // counters split correctly in the dashboards.
