...
# ── Helm install (placeholder — chart not yet generated) ──────────────────────
helm_install() {
  hdr "Helm install"
  # =========================================================================
  # TODO: HELM CHART NOT YET GENERATED
  #
  # When helm/Chart.yaml exists, this section should:
  #   1. Convert .env to a values.yaml (or pass values via --set-string)
  #   2. helm upgrade --install ai-o11y-demo-apps "${HELM_DIR}" \
  #          --create-namespace \
  #          --values "${HELM_DIR}/values.yaml" \
  #          --values "${REPO_ROOT}/.env.values.yaml" \
  #          --wait --timeout 10m
  #
  # For now we just confirm the chart directory state and exit.
  # =========================================================================
  if [[ -f "${HELM_DIR}/Chart.yaml" ]]; then
    warn "Helm chart exists at ${HELM_DIR} but the install step in this script is still a TODO."
    warn "Run the helm command manually for now, or update install.sh once the chart is final."
  else
    warn "Helm chart not yet generated. Skipping helm install."
    warn "Would have run:"
    warn "  helm upgrade --install ai-o11y-demo-apps ${HELM_DIR} \\"
    warn "    --create-namespace --values <generated-values.yaml>"
  fi
}
